...
/* fun text(){
        ...
        //checking image width and heigth to be same
        if(image1.width != image2.width || image1.height != image2.height){
        println("Pictures sizes don't match")
        //return emptyList()
    }

        need certain sizes of shots to decide which pixels I can Ignore as well as 
        NEED CROP FEATURE FOR EACH TARGET
        a second comparison that takes either new colored image and compares to another at a different threshhold 
        or a second comparison that finds the same pixels between two white images of different thresholds

        
        validation goes a long way ie 
        processing time(Online_Tar <Targ1, Targ2>) = 5.55 avg time seconds; threshold = 100

        processing time(Target_clear -> Target_Clear) = 8.23 seconds; threshold = 100 needs to be higher 40k pixels appeared
        processing time(Target_clear -> Target_Clear) = 8.1 seconds; threshold = 110 needs to be higher 4106 pixels appeared
        processing time(Target_clear -> Target_Clear) = 8.04 seconds; threshold = 120 needs to be higher 1471 pixels appeared
        processing time(Target_clear -> Target_Clear) = 8.379 seconds; threshold = 130 needs to be higher 387 pixels appeared
        processing time(Target_clear -> Target_Clear) = 8.007 seconds; threshold = 140 needs to be higher 58 pixels appeared
        processing time(Target_clear -> Target_Clear) = 7.94 seconds; threshold = 150 needs to be higher 3 pixels appeared
        processing time(Target_clear -> Target_Clear) = 7.657 seconds; threshold = 155; Pictures are the same min threshold is 152
    
        processing time(Target_clear -> First_Shot) = 8.035 seconds; threshold = 150 needs to be higher 550k pixels appeared
        processing time(Target_clear -> First_Shot) = 8.149 seconds; threshold = 160 needs to be higher 453k pixels appeared jumping to 200
        processing time(Target_clear -> First_Shot) = 8.149 seconds; threshold = 200 needs to be higher 153k pixels appeared 
        processing time(Target_clear -> First_Shot) = 8.149 seconds; threshold = 210 needs to be higher 153k pixels appeared 
        issue shot is on red color, therefore threshold is considered to high to high attempting grayscaling 
        issue still appears in grayscale shot was barely appearing at theshold 100

        set a range for the differences therefore it doesn't register anything larger than one value and lower than another
         */


        //future notes for 404
        //I need code that will grab the pixel value with that y and then once those are found I can pin the middle of the shot to get used for stats
 

        /*validation goes a long way ie 


        error percentage = like how far off am Ie
  }*/
